...
\vfil\break
\begingroup \parskip=1ex
\hrule
\noindent The next sequence of examples illustrates some
...
McCawley (1982) seems to be the first linguist to attempt
to use multidominance to explain a complex syntactical
phenomenon, so we begin with one of his examples (right node
raising). \endgroup
\smallskip
\hrule
...
\exdisplay
\hfil
\jtree[xunit=2.45em,yunit=3ex,angleA={(1,-1)},nodesep=0]
\def\\{[labelgapb=-4pt]}%
\def\V{$\rm \overline V$}%
\! = {S}
